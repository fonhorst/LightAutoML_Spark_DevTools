...
class DistributedLightFMWrap(HybridRecommender, PartialFitMixin, HnswlibMixin):
    ...
    def _reinitialize_embeddings(
        ...
        def _concat_arrays(
            model_array: np.ndarray, added_array: np.ndarray, dissect_at: int
        ):
    ...
    def _fit_partial(
        ...
        user_features = self._convert_features_to_csr(
            log.select("user_idx").distinct(), user_features
        )
        item_features = self._convert_features_to_csr(
        ...
        (
            user_features,
            item_features,
        ) = self.model._construct_feature_matrices(
        ...
        training_instance = LightFMTraining(
            self.model, self.world_size, self.num_threads
        )
        ...
        def udf_to_map_on_interactions_with_index(
            ...
            context = pgl.rendezvous.Context(
                p_idx, training_instance.world_size
            )
            ...
            # Prapare data
            interactions = training_instance.rdd_to_csr(
                partition_interactions, num_users=n_users, num_items=n_items
            )
            ...
            for _epoch_num in training_instance.model._progress(
                _num_epochs, verbose=False
            ):
        ...
        self.model = interactions.rdd.mapPartitionsWithIndex(
            udf_to_map_on_interactions_with_index
        ).collect()[0]
