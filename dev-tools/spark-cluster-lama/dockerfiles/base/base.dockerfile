...
RUN pip3.9 install py4j

RUN PYTHONPATH="$PYTHONPATH;/opt/bitnami/spark/python" python3.9 -c 'from pyspark.sql import SparkSession; SparkSession.builder.config("spark.jars.packages", "com.microsoft.azure:synapseml_2.12:0.9.5").config("spark.jars.repositories", "https://mmlspark.azureedge.net/maven").getOrCreate()'
RUN mv /root/.ivy2/jars /usr/local/lib/jars_synapseml_0.9.5

RUN PYTHONPATH="$PYTHONPATH;/opt/bitnami/spark/python" python3.9 -c 'from pyspark.sql import SparkSession; SparkSession.builder.config("spark.jars.packages", "com.microsoft.azure:synapseml_2.12:0.9.4").config("spark.jars.repositories", "https://mmlspark.azureedge.net/maven").getOrCreate()'
RUN mv /root/.ivy2/jars /usr/local/lib/jars_synapseml_0.9.4

#COPY build_tmp/requirements* /tmp
#
#RUN test -f /tmp/requirements-3.8.txt && pip3.8 install -r /tmp/requirements-3.8.txt
#RUN test -f /tmp/requirements-3.9.txt && pip3.9 install -r /tmp/requirements-3.9.txt
